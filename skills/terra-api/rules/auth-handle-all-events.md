...
```typescript
switch (event.type) {
  case "auth":              return upsertConnection(terraUserId, { status: "active", provider, scopes: parseScopes(event.user?.scopes) });
  case "auth_cancelled":    return console.log(`terra auth cancelled for ${event.reference_id}: ${event.reason}`);
  case "deauth":
  ...
  case "permission_change": return applyScopeDelta(terraUserId, parseScopes(event.scopes_added), parseScopes(event.scopes_removed));
}
```
