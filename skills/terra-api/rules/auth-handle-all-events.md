---
title: Handle All Six Auth Event Types
impact: HIGH
impactDescription: missed lifecycle events leave zombie connections
tags: auth, event-types, connections
---

## Handle All Six Auth Event Types

**Impact: HIGH (missed lifecycle events leave zombie connections)**

Connection state changes arrive as webhooks, and there are six auth event types, not one. Handling only `auth` and `deauth` leaves connections showing "active" after the provider revoked access, breaks silently when Terra API rotates the user ID on reauth, and never updates scopes. Key all updates on a unique constraint over the Terra user ID so handlers are idempotent upserts.

| Event | What to do |
|---|---|
| `auth` | Upsert connection with status `active`, set provider and scopes |
...
// access_revoked, user_reauth, connection_error, permission_change: ignored
```
...
```typescript
switch (event.type) {
  case "auth":              return upsertConnection(terraUserId, { status: "active", provider, scopes });
  case "deauth":
