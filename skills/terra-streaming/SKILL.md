...
- **Close codes split into two groups.** 4000, 4003, 4004, and 1003 signal a client bug – fix the client, do not retry-loop, since a blind reconnect just loops on the same error. 4001 means mint a fresh token. 4002 means the consumer session cap was hit; close the other session first. The cap is currently 1 but is a server-side setting that may rise, so do not hard-code an assumption of exactly one consumer.
...
- **REPLAY requires both `after` and `before`.** Bounds are exclusive. Omitting `before` returns nothing. Set `before` to the `seq` of the first live DISPATCH after reconnect to backfill exactly the gap.
