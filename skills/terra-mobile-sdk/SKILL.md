...
- **Apple Health can produce multiple Terra user IDs for one person.** Connections key on the tuple `(deviceId, devId, referenceId)`. The same person on a second device, or after a reinstall that reset the vendor ID, gets a **new** Terra `user_id` sharing the same `reference_id`. Terra API deliberately does **not** auto-link these, because HealthKit data is device-local and may legitimately differ per device. Deduplicate on your side by grouping on `reference_id`, or gate `initConnection` to a single primary device.
...
- **Multi-device gating and dedup strategy.** Whether to allow every device to connect (and dedup on `reference_id`) or designate one primary device.
